...
def TrainSTRidge(R, Ut, lam, d_tol, maxit = 25, STR_iters = 10, l0_penalty = None, normalize = 2, split = 0.8, print_best_tol = False):
    ...
    err_best = np.linalg.norm(TestY - TestR.dot(w_best), 2) + np.count_nonzero(w_best)
    tol_best = 0
    ...
    for j in range(maxit):
        ...
        if lam != 0: 
            w[biginds] = np.linalg.lstsq(X[:, biginds].T.dot(X[:, biginds]) 
                                         + lam*np.eye(len(biginds)),X[:, biginds].T.dot(y),rcond=None)[0]
        else: 
...
def aic(pt,learned_pt,k):
    ...
    aic = m*np.log(rss/m)/k + 2*k
    
    return aic


def learned_model(c1,c2,c3,c4,c5,time=27):
    ...
    #solve the PDE
    res = eq.solve(field, time, dt=0.01, tracker=["progress", storage.tracker(1.0)])  #  ["progress", storage.tracker(0.0001)]
...
def pdefind(X, y, epsilon = 0.1, maxit_f = 100, maxit_b = 5, backwards_freq = 5, relearn_f=True, relearn_b=True):
    """
    Forward-Backward greedy algorithm for sparse regression.
    See Zhang, Tom. 'Adaptive Forward-Backward Greedy Algorithm for Sparse Learning with Linear Models', NIPS, 2008
    The original version of this code that was uploaded github was contained errors.  This version has been corrected and
